...
export function useBanners(params: { page?: number; pageSize?: number; position?: string; status?: string }) {
  return useQuery({
    queryKey: ['banners', params],
    queryFn: async () => { const res = await getBanners(params); return res.data; },
  });
}
...
export function useBanner(id: string) {
  return useQuery({
    queryKey: ['banner', id],
    queryFn: async () => { const res = await getBanner(id); return res.data; },
    enabled: !!id,
  });
}
...
export function useUpdateBanner() {
  const invalidate = useInvalidateBanners();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Record<string, unknown> }) => updateBanner(id, data),
