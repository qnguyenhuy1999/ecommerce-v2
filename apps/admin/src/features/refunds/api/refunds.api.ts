...
export interface RefundListItem {
  ...
  timeline: { id: string; fromStatus: string; toStatus: string; note: string | null; createdAt: string }[];
}
...
export async function getRefundStatusCounts() {
  return apiFetch<{ success: boolean; data: Record<string, number> }>('/admin/refunds/status-counts');
}
