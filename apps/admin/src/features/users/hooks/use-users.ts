...
import { getUsers, getUser, getUserStatusCounts, suspendUser, banUser, activateUser } from '../api/users.api';

export function useUsers(params: { page?: number; pageSize?: number; search?: string; status?: string }) {
  return useQuery({
    queryKey: ['users', params],
    queryFn: async () => { const res = await getUsers(params); return res.data; },
  });
}
...
export function useUser(id: string) {
  return useQuery({
    queryKey: ['user', id],
    queryFn: async () => { const res = await getUser(id); return res.data; },
    enabled: !!id,
  });
}
...
export function useUserStatusCounts() {
  return useQuery({
    ...
    queryFn: async () => { const res = await getUserStatusCounts(); return res.data; },
  });
}
