...
export default function MetricsPage() {
  ...
  const getRateColor = (rate: number, inverse = false) => {
    if (inverse) return rate >= 90 ? 'text-green-600' : rate >= 70 ? 'text-yellow-600' : 'text-red-600'
          ...
          {history.length > 0 && (
            <div className="bg-white rounded-lg border border-gray-200 p-6">
              ...
              <div className="space-y-2">
                {history.map((snap) => (
                  <div key={snap.id} className="flex items-center justify-between text-sm border-b border-gray-100 py-2">
                    <span className="text-gray-600">{new Date(snap.date).toLocaleDateString()}</span>
