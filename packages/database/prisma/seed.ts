...
async function main() {
  ...
  const adminRoles = [
    ...
    },
    {
      ...
      permissions: ['SELLER_VIEW', 'ORDER_VIEW', 'DASHBOARD_VIEW'] as const,
    },
    {
      name: 'VIEWER' as const,
      description: 'Read-only access',
      permissions: ['SELLER_VIEW', 'PRODUCT_VIEW', 'ORDER_VIEW', 'DASHBOARD_VIEW'] as const,
    },
  ]
  ...
  console.log('Seeded admin roles:', adminRoles.map((r) => r.name).join(', '))
    ...
    if (superAdminRole) {
      const admin = await prisma.admin.create({
        data: {
          ...
          passwordHash: '$2b$12$LJ3m4ys3uz2RmVfGHpOHxOsJMtL2F7.UGHb2kQYpBLKJSp9oQOLCe',
          firstName: 'Super',
          lastName: 'Admin',
          status: 'ACTIVE',
        },
